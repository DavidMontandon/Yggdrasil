...
public class VanillaDeepFrozenOceanBiome extends Biome {
	protected static final PerlinNoiseGenerator field_206856_bb = new PerlinNoiseGenerator(new SharedSeedRandom(3456L),
			2, 0);

	public VanillaDeepFrozenOceanBiome() {
		super((new Biome.Builder()).surfaceBuilder(SurfaceBuilder.FROZEN_OCEAN, SurfaceBuilder.GRASS_DIRT_GRAVEL_CONFIG)
				.precipitation(Biome.RainType.RAIN).category(Biome.Category.OCEAN).depth(-1.8F).scale(0.1F)
				.temperature(0.5F).downfall(0.5F).waterColor(3750089).waterFogColor(329011).parent((String) null));
		this.addStructure(
				Feature.OCEAN_RUIN.withConfiguration(new OceanRuinConfig(OceanRuinStructure.Type.COLD, 0.3F, 0.9F)));
	...
	public float getTemperatureRaw(BlockPos pos) {
		...
		if (pos.getY() > 64) {
			float f1 = (float) (TEMPERATURE_NOISE.noiseAt((double) ((float) pos.getX() / 8.0F),
					(double) ((float) pos.getZ() / 8.0F), false) * 4.0D);
